...
func Initialize(ctx context.Context, scope string) error {
	...
	createCounter, _ := meter.Int64Counter("art_design_pro_go_apiserver_resource_create_total", metric.WithDescription("Total number of REST resource create requests"))
	getCount, _ := meter.Int64Counter("art_design_pro_go_apiserver_resource_get_total", metric.WithDescription("Total number of REST resource get requests"))
